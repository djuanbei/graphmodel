...
int UppaalParser::parserDeclaration(  child_type declarations){
  ...
  for( child_iterator it= declarations->begin( ); it!=
                                                    declarations->end( ); it++){
...
int UppaalParser::parserTemplate( child_type templates){
  ...
  for( child_iterator it= templates->begin( ); it!=
                                                    templates->end( ); it++){
    ...
    for( child_iterator lit= locations->begin( ); lit!= locations->end( ); lit++){
      ...
      if( NULL!= labels){
        for( child_iterator llit= labels->begin( ); llit!= labels->end( ); llit++  ){
          string kind="";
    ...
    for( child_iterator tit= transitions->begin( ); tit!= transitions->end( ); tit++){
...
int UppaalParser::parserQuery(  child_type queries){
  return 0;
}
