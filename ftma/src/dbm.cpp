...
namespace graphsat {
  ...
  DBMFactory::DBMFactory( int n, const vector<int> &oclockUppuerBound,
                         const vector<ClockConstraint> &odifferenceCons ) : clock_num( n + 1 ) {
    matrix_size = clock_num * clock_num;
  ...
  bool DBMFactory::isSatisfied( const int *const dbm, const ClockConstraint &cons ) const {
  ...
  int *DBMFactory::corn_norm( int *dbm, const vector<int> &maximums,
