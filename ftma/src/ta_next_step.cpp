    ...
    if (manager->hasMatchOutUrgentChan(state)) {
      ...
      for (int i = 0; i < component_num; i++) {
        ...
        for (auto link : out_ts) {
          ...
          if (!sys.transitionReady(i, link, state)) {
            continue;
          }
          OneStep dummy;
...
void TANextStep::doUrgant(int *state, std::vector<OneStep> &re) const {
  ...
  for (int i = 0; i < component_num; i++) {
    int loc_a = manager->getLocationID(i, state);
    vector<int> dummy_a = sys.getOutUrgent(i, loc_a, counter_value);
    set<int> temp(dummy_a.begin(), dummy_a.end());
    for (int j = i + 1; j < component_num; j++) {
      int loc_b = manager->getLocationID(j, state);
      vector<int> dummy2 = sys.getOutUrgent(j, loc_b, counter_value);
      for (auto e : dummy2) {
