...
private:
  ...
  bool isReach( const vector<CS> &cons, const C *const dbm ) const {
    if ( cons.empty() ) {
      return true;
    }
...
public:
  ...
  bool run( const int loc, const vector<CS> &cons ) {
    ...
    //    ASSERT(initial_loc>=0, "The initial location must greater or equal
    //    to 0" );
    ...
    while ( !changed.empty() ) {
      ...
      for ( map<int, vector<int>>::iterator it = relatedLinks.begin();
            it != relatedLinks.end(); it++ ) {
      ...
      changed.clear();
      changed.insert( changed.begin(), secondChanged.begin(),
                      secondChanged.end() );
      secondChanged.clear();
  ...
  void deleteData() {
    for ( typename vector<dbmset<C, DBM>>::iterator it = reachSet.begin();
          it != reachSet.end(); it++ ) {
      it->deleteAll();
    }
    reachSet.clear();
