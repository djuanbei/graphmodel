...
class CounterConstraint{
 private:
  ...
  COMP_OPERATOR op;
 public:
  CounterConstraint( const vector<pair<int, int> >  &cons, int erhs, COMP_OPERATOR eop ):constraint( cons), rhs(erhs ), op( eop){
    
  }
  
  bool operator ( ) ( const vector<int> & valuation ) const{
    int dummy=0;
    for( vector<pair<int, int> >::const_iterator it= constraint.begin( ); it!= constraint.end( ); it++){
