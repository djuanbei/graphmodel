...
class dbm{
 ...
 public:
  ...
  bool equalM(const C* lhs, const C* rhs) const{
    ...
    for( typename vector<Cons>::const_iterator cit=Gd.begin( ); cit!=Gd.end( ); cit++ ){
      ...
      for ( typename vector<C*>::iterator dit =re.begin( ); dit!= re.end( ); dit++ ){
        ...
        if( isSatisfied(*dit, *cit) && isSatisfied( *dit, (*cit).neg( ))){
          ...
          map<uint32_t, C*>::iterator fid1=passed.find(h1);
          map<uint32_t, C*>::iterator fid2=passed.find(h2);
          ...
          map<uint32_t, C*>::iterator fid= passed.find(h);
